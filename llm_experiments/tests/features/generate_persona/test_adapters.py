...
def test_censo_adapter_race(sidra_client, db_cache):
    ...
    expected_categories = {"Branca", "Preta", "Amarela", "Parda", "Indígena", "Sem declaração"}
...
def test_civil_adapter_marital_status(sidra_client, db_cache):
    ...
    expected_categories = {"Solteiro(a)", "Casado(a)", "Divorciado(a)", "Viúvo(a)", "Desquitado(a) ou separado(a) judicialmente"}
    for label in dist.keys():
        assert label in expected_categories or "solteir" in label.lower() or "casad" in label.lower()
...
def test_pns_adapter_health_assessment(sidra_client, db_cache):
    ...
    expected_categories = {"Muito bom", "Bom", "Regular", "Ruim", "Muito ruim", "Muito boa", "Boa"}
    for label in dist.keys():
        assert label in expected_categories or "bom" in label.lower() or "ruim" in label.lower() or "regular" in label.lower()
